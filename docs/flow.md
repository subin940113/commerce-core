...
| 구분 | 내용 |
|------|------|
| 동작 | `(provider, providerEventId)` 유니크 기반 insert-first 전략, 먼저 insert 시도·UNIQUE 위반 시 이미 처리된 이벤트로 간주 후 현재 상태 반환, AUTHORIZED → 재고 확정·Outbox 적재·FAILED → 예약 해제만 |
...
```mermaid
flowchart LR
    A["PENDING 조회<br/>FOR UPDATE SKIP LOCKED"] --> B[RabbitMQ publish]
    B --> C[PUBLISHED]
    B --> D[실패 시 재시도]
    D --> E["FAILED<br/>N회 초과"]
```
...
| 구분 | 내용 |
|------|------|
| 동작 | PENDING 이벤트 배치 조회(`FOR UPDATE SKIP LOCKED`) → RabbitMQ publish → 성공 시 PUBLISHED, 실패 시 재시도·N회 초과 시 FAILED 상태 유지, 반복 실패 메시지는 DLQ로 분리 |
...
MVP에서는 반복 실패 메시지를 `shipping.payment-authorized.dlq`로만 분리해 두며, DLQ를 구독·처리하는 애플리케이션 코드는 없다. 추후 아래 옵션 중 필요에 맞게 검토하면 된다.

| 방안 | 설명 |
|------|------|
| 운영 확인 | RabbitMQ Management에서 DLQ 큐·메시지 조회, 필요 시 수동 재발행 또는 로그/메트릭 연동 |
| 재처리 | DLQ 전용 리스너에서 재시도 정책·백오프 적용 후 원래 큐 또는 재처리 전용 큐로 재발행 |
| 알림 | DLQ 메시지 적재 시 슬랙·이메일 등으로 알림 전송, 수동 개입 유도 |
